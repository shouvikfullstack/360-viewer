...
describe('Viewer', () => {
	it('renders with enough images', () => {
		const { container } = render(<Viewer images={mockImages} />);
		expect(container.querySelectorAll('img').length).toEqual(36);
	});

	it('shows next image', () => {
		const ViewerRef = createRef();
		const { container } = render(<Viewer ref={ViewerRef} images={mockImages} />);
		expect(container.querySelector('.Viewer-viewer-image-shown').src).toEqual(
			'https://i.ibb.co/NTRYMDT/1.jpg'
		);
		act(() => {
	...
	it('shows next image inverse', () => {
		const ViewerRef = createRef();
		const { container } = render(<Viewer ref={ViewerRef} images={mockImages} inverse />);
		expect(container.querySelector('.Viewer-viewer-image-shown').src).toEqual(
			'https://i.ibb.co/NTRYMDT/1.jpg'
		);
		act(() => {
